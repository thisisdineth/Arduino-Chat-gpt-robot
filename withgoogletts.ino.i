...
void textToSpeech(String text) {
  HTTPClient http;
  String ttsURL = "https://texttospeech.googleapis.com/v1/text:synthesize?key=" + String(GOOGLE_CLOUD_API_KEY);
  http.begin(ttsURL);
  http.addHeader("Content-Type", "application/json");
  ...
  payload["voice"]["name"] = "en-US-Wavenet-D"; // Choose a specific voice
  payload["audioConfig"]["audioEncoding"] = "LINEAR16";
  ...
  if (httpResponseCode == 200) {
    ...
    File audioFile = SD.open("/response.wav", FILE_WRITE);
    if (audioFile) {
      for (size_t i = 0; i < audioContent.length(); i += 2) {
        uint8_t byte = strtoul(audioContent.substring(i, i + 2).c_str(), NULL, 16);
        audioFile.write(byte);
      }
      audioFile.close();
      playAudio("/response.wav");
    }
  } else {
    Serial.printf("Error in TTS: %d\n", httpResponseCode);
  }
  http.end();
}
